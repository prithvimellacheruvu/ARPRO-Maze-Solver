...
{
public:
    typedef std::unique_ptr<Point> PositionPtr;
    ...
    virtual void print(const Point &parent, bool isCorridor);
    void childFinder(int &x1, int &y1, int dir, std::vector<PositionPtr> &generated);
    int checker(int x1, int y1);
    void move2FreeSpace(int &x1, int &y1, int &dir);
