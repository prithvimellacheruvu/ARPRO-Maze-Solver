...
public:
    ...
    std::vector<Node> fullPathTo(Node* best, int dist, bool method) const
    {
...
template<class Node>
std::vector<Node> Astar(Node start, Node goal, bool method)
{
    ...
    while(!queue.empty())
    {
        ...
        if(areSame(best.node,goal))
        {
            ...
            return tree.fullPathTo(best.node, best.g, method);
        }
